...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if state in self.qValues:
	        ...
	        value = max(self.getQValue(state, action) for action in self.getLegalActions(state))
    ...
    def getAction(self, state):
        ...
        if random:
        	...
        	possibleStates = [action for action in legalActions]
    ...
    def update(self, state, action, nextState, reward):
        ...
        if state in self.qValues:
        	...
        	self.qValues[state].append((action, newQValue))

        	for q in self.qValues[state]:

        		if q[0] == action:

        			self.qValues[state].remove(q)
